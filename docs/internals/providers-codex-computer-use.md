...
The T3-owned route and its remaining release gates are defined in the
[T3 Computer Use specification](./computer-use.md). This document describes only the OpenAI bridge
...
- through this bridge, T3 cannot grant, revoke, or display the authoritative macOS permission state;
- the separate T3-owned route uses T3's native helper, host protocol, and policy instead;
...
- The bridge does not manage macOS Accessibility or Screen Recording permissions. The signed client
  owns that setup and reports pending or denied permission states through its tool results.
...
| Sensitive-action confirmations                  | Partial         | The bridge can project Codex requests, but it does not route them through T3's native Computer Use risk policy.                                           |
| Stop and take over                              | Partial         | A bridge provider turn can be interrupted, but it does not use the native route's dedicated takeover lifecycle.                                           |
...
| Computer Use settings and permission management | Missing         | The bridge has only its per-provider enable switch; T3-owned grants, history, browser access, and lifecycle controls belong to the separate native route. |
| Locked-computer use                             | Missing         | T3 has no trusted unlock or locked-use implementation.                                                                                                    |
| Windows computer use                            | Missing         | The bridge discovers only the signed macOS client; Windows support belongs to the T3-owned toolkit.                                                       |
...
| Browser extension/semantic browser control      | Missing         | Native GUI accessibility can target a browser, but this bridge does not expose T3's separately governed preview and persistent-profile browser routes.    |
